                ...
                <transition name="activeCard">
                    <div v-if="activeParty">
                        <active-party-element
                            ...
                            :description="
                                activeParty && activeParty.summaryBody
                            "
...
export default {
    name: 'PartyView',
    setup() {
        const { partyMainPageContent, mainPageError, loadMainContent } =
            getPartyMainPage();
        ...
        const mainInfo = computed(() =>
            partyMainPageContent.value.filter(
                content => content.page === 'Main Page Info',
            ),
        );

        const partyList = computed(() =>
            partyMainPageContent.value.filter(
                content => content.page === 'Detail Page',
            ),
        );

        const activateParty = partyId => {
            activeParty.value = partyMainPageContent.value.find(
